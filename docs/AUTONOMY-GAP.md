...
| Operator interface | UNWIRED | The resource console includes a pinned-project task workspace with opt-in durable private task transcripts, explicit retention/deletion, text attachments, output, cancellation and a resizable inspector. Retention preserves ledger/task identity; missing captured output is never regenerated. Unsent drafts are not persisted and auth/scope changes clear private UI state. Multi-turn context, project switching, terminal/browser/file panels and resident-firm controls remain unwired. |
