...
| Competing plans and artifact graph | UNWIRED | `control-graph.ts` persists signed intent/settlement and artifact dependencies. `firm-resource-control-handler.ts` connects enrolled response generation. `firm-engineering-control-handler.ts` and `universe firm engineer` now connect explicit host enrollment to the existing portfolio controller, resource-accounted confined file operations, fixed evaluation and strict-improvement local branch delivery. Resident planning, uncertain graph-intent recovery and live commissioning remain unwired. |
