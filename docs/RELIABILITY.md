...
| Operator-invoked protected-PR handoff | Available, never silent | `ashlr inbox submit` requires caller confirmation, signed frontier provenance, fresh exact base/diff-bound verification, enrollment, kill-switch clear, and live protected-remote evidence. It opens one review PR and never merges the default branch or invokes a model. `--yes` records caller intent; it is not an authenticated human identity or durable approval receipt. |
