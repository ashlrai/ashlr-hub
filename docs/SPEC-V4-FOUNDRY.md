...
| Fleet control plane + observability | `ashlr fleet status/pause/resume` + `/api/fleet` + `#fleet` web view: per-backend throughput, queue, merges, proposals, quota, one-button pause/kill | M49 ✓ |
...
| **M47.1** | **Signed provenance** — HMAC-bind `{engineModel, engineTier, diffHash}` at sandboxed-producer time and re-verify in the merge gate, so a local/in-process writer can't forge frontier merge-authority on a proposal record (review finding H3). The merge gate now FAIL-CLOSED requires a valid HMAC over `${engineModel}|${engineTier}|sha256(scrubbed diff)` from the per-machine key (`~/.ashlr/foundry/provenance.key`, 0600); a forged or tampered proposal record can no longer claim frontier authority. | merge authority can't be spoofed on disk | S–M | **Shipped** |
...
| **M49** | **Fleet control plane + observability** — live dashboard/CLI: per-backend throughput, queue, merges-to-main, per-repo health trends, per-tier budgets/quotas, pause/kill. Extends the web UI. | one glance at the whole fleet; one button to stop it | M | **Shipped** |
