...
describe('3.3.0 authority release truth', () => {
  ...
  it('records the shipped console and web security changes without an unchanged-backend claim', () => {
    const release = releaseBlock('3.3.0');

    expect(release).toContain('session-bound and expiry-bound SSE');
  ...
  it('keeps setup, update, and help guidance on admitted owner or dry-run paths', () => {
    ...
    expect(release).toMatch(/report-only learning signals/i);
