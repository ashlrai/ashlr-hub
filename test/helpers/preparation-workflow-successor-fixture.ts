/** Trusted real two-generation source delivery. No model/provider calls and no successor execution. */
...
export async function preparationSuccessorFixture(base: string) {
  ...
  const campaign = await runUniverseCampaign('upstream-campaign', { root: sourceRoot });
