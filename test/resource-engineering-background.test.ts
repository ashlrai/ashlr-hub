...
afterEach(() => { fixture.workers.splice(0); vi.clearAllMocks(); });
...
describe('engineering background ownership and drain protocol', () => {
  ...
  it('bounds pending commands and allows the distinct close request through the full queue', async () => {
    ...
    const reads = Array.from({ length: 8 }, () => background.snapshot().catch(() => null));
    await expect(background.snapshot()).rejects.toThrow('unavailable');
    const closing = background.close();
    for (const message of worker.messages.filter(message => message.kind === 'snapshot')) worker.emit('message', {
      type: 'engineering-result', id: message.id, ok: false, code: 'UNAVAILABLE',
    });
    reply(worker, 'close'); await closing; await Promise.all(reads);
    expect(worker.messages.filter(message => message.kind === 'snapshot')).toHaveLength(8);
  });
  it('vetoes automatic admission while paused and forwards one exact CAS without retry', async () => {
    ...
    const configured = background.configureSuccessors({} as Parameters<EngineeringBackground['configureSuccessors']>[0],
      supervision as unknown as Parameters<EngineeringBackground['configureSuccessors']>[1],
      () => ({ observations: [], unavailableWorkerIds: [] }));
    reply(worker, 'configure-successors'); await configured;
