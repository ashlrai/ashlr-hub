...
function completeBundle(input: {
  ...
  const bundleRoot = join(input.parent, `${input.role}-bundle`);
  const packageRoot = join(bundleRoot, input.revision);
  mkdirSync(packageRoot, { recursive: true });
  writeText(join(packageRoot, 'package.json'), `${JSON.stringify({
    name: '@ashlr/hub',
    version: '3.2.0',
    type: 'module',
    bin: { ashlr: 'bin/ashlr' },
  ...
  writeText(join(packageRoot, 'package-lock.json'), `${JSON.stringify({
    name: '@ashlr/hub',
    version: '3.2.0',
    lockfileVersion: 3,
    packages: {
  ...
  const evidenceTrustRootCanonicalSha256 =
    runtimeReleaseTrustRootCanonicalSha256(input.evidenceTrustRoot);
...
function fixture(options: { minimumPolicyEpoch?: number; policyEpoch?: number } = {}): Fixture {
  ...
  return { home, now, privateKey, publicKey, trustRoot, payload, request, requestPath };
}
...
describe('M502 runtime activation authority', () => {
  ...
  it('verifies operator custody and signature before refusing absent immutable bundles', () => {
    ...
    expect(result.trust).toMatchObject({ operatorCustodyVerified: true, signatureVerified: true });
    ...
    expect(result.authorityBlockers).toContain('durable-replay-consumption-absent');
    assertNoMutationAuthority(result);
  });
  ...
  it('rejects a signature made by a key outside the operator root', () => {
    ...
    const result = preflightRuntimeActivationAuthority({ homePath: f.home, nowMs: f.now, requestPath: f.requestPath });
    expect(result.blockers).toEqual(expect.arrayContaining([
      expect.objectContaining({ code: 'signed-manifest-invalid' }),
    ]));
    assertNoMutationAuthority(result);
  });
  ...
  it('rejects a plan filename that is not the signed plan id', () => {
    ...
    const result = preflightRuntimeActivationAuthority({ homePath: f.home, nowMs: f.now, requestPath: wrong });
  ...
  it('rejects permissive trust-root and request modes', () => {
    ...
    let result = preflightRuntimeActivationAuthority({ homePath: f.home, nowMs: f.now, requestPath: f.requestPath });
    expect(result.blockers[0]).toMatchObject({ code: 'operator-trust-root-unavailable' });
    chmodSync(paths.trustRootPath, 0o600);
    chmodSync(f.requestPath, 0o644);
    result = preflightRuntimeActivationAuthority({ homePath: f.home, nowMs: f.now, requestPath: f.requestPath });
    expect(result.blockers[0]).toMatchObject({ code: 'request-unavailable' });
    assertNoMutationAuthority(result);
  });
  ...
  it.skipIf(process.platform === 'win32')('rejects symlink and hard-link plan substitution', () => {
    ...
    let result = preflightRuntimeActivationAuthority({ homePath: f.home, nowMs: f.now, requestPath: symlink });
    expect(result.blockers[0]).toMatchObject({ code: 'request-unavailable' });
    rmSync(symlink);
    const hardlink = join(paths.plansPath, '123e4567-e89b-42d3-a456-426614174002.json');
    linkSync(f.requestPath, hardlink);
    result = preflightRuntimeActivationAuthority({ homePath: f.home, nowMs: f.now, requestPath: hardlink });
    expect(result.blockers[0]).toMatchObject({ code: 'request-unavailable' });
    assertNoMutationAuthority(result);
  });
  ...
  it('derives deterministic observation-only replay evidence from signed bytes', () => {
    ...
    expect(runtimeActivationPlanEvidenceCanonicalJson(first)).toBe(
      runtimeActivationPlanEvidenceCanonicalJson(second),
    );
  });
  ...
  it('publishes deterministic plan evidence with durable no-clobber semantics', () => {
    ...
    const storage = { anchorPath: anchor, storeRoot: join(anchor, 'plan-evidence') };
