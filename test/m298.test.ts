...
// ---------------------------------------------------------------------------
// 1. STREAM-JSON — claude argv uses stream-json + --verbose
// ---------------------------------------------------------------------------

describe('M298 Part 1 — stream-json argv', () => {
  const cfg = baseConfig();

  it('builtin claude entry uses --output-format stream-json, not json', () => {
    const spec = BUILTIN_ENGINE_REGISTRY['claude']!;
    expect(spec.argv).toBeDefined();
    // argv must contain stream-json
  ...
  it('normaliseEngineOutputLine parses a stream-json content_block_start as tool_call', async () => {
    // Import engines.ts via dynamic import (it exports normaliseEngineOutputLine indirectly
    // through spawnEngine; we test the observable behaviour by verifying the registered
    // stream-json argv, since normaliseEngineOutputLine is not exported).
    // Instead we verify via the RunEvent structure using the documented output path.
    // The key invariant: a stream-json line with type:'content_block_start' and
    // content_block.type:'tool_use' should be recognised.
    // We call normaliseEngineOutputLine indirectly by checking the BUILTIN spec is stream-json
