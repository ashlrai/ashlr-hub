...
const EXPECTED_PREVIOUS_CANDIDATE_VERSION = '3.2.7';
...
describe('M522 — production-promotion admission has no npm mutation authority', () => {
  it('binds the promotion, release workflow, and package to the exact 3.3.0 identity', () => {
    ...
    expect(EXPECTED_TAG).toBe('v3.3.0');
  });

  it.each([
    [
      'promotion workflow version',
      promotionText.replace('PROMOTION_VERSION: "3.3.0"', 'PROMOTION_VERSION: "3.3.1"'),
      releaseText,
      packageText,
    ],
    [
      ...
      promotionText,
      releaseText.replace('RELEASE_VERSION: "3.3.0"', 'RELEASE_VERSION: "3.3.1"'),
      packageText,
    ],
    [
      'package version',
      promotionText,
      releaseText,
      JSON.stringify({ ...packageMetadata, version: '3.3.1' }),
    ],
    [
      'candidate dist-tag',
    ...
    [
      ...
      promotionText,
      releaseText.replace('PREVIOUS_CANDIDATE_VERSION: "3.2.7"', 'PREVIOUS_CANDIDATE_VERSION: "3.2.6"'),
      packageText,
    ],
    [
      'exact release tag',
    ...
    [
      ...
      promotionText,
      releaseText.replace('ashlr-hub-3.3.0.tgz', 'ashlr-hub-next.tgz'),
      packageText,
    ],
  ...
  it('uploads only the bounded receipt and digest with an immutable action', () => {
    ...
    expect(admission?.run).toContain('--arg promotionCommand "npm dist-tag add @ashlr/hub@3.3.0 latest"');
  ...
  it.each([
    ['command prefix', '\ncommand npm dist-tag add @ashlr/hub@3.3.0 latest'],
    ['env prefix', '\nenv npm dist-tag add @ashlr/hub@3.3.0 latest'],
    ['env option separator', '\nenv -- npm dist-tag add @ashlr/hub@3.3.0 latest'],
    ['absolute npm path', '\n/usr/bin/npm dist-tag add @ashlr/hub@3.3.0 latest'],
    ['npm registry option', '\nnpm --registry https://registry.npmjs.org dist-tag add @ashlr/hub@3.3.0 latest'],
    ['exec wrapper', '\nexec /usr/bin/npm dist-tag add @ashlr/hub@3.3.0 latest'],
    ['npm executable variable', '\nnode "$PROMOTION_NPM_CLI" dist-tag add @ashlr/hub@3.3.0 latest'],
    ['npm CLI path', '\nnode /opt/npm/lib/node_modules/npm/bin/npm-cli.js dist-tag add @ashlr/hub@3.3.0 latest'],
    ['corepack npm wrapper', '\ncorepack npm dist-tag add @ashlr/hub@3.3.0 latest'],
    ['npm line continuation', '\nnpm \\\n  dist-tag add @ashlr/hub@3.3.0 latest'],
    ...
    ['shell command wrapper', "\nbash -c 'npm dist-tag add @ashlr/hub@3.3.0 latest'"],
    ['function wrapper', '\npromote() { "$@"; }\npromote npm dist-tag add @ashlr/hub@3.3.0 latest'],
  ...
  it('does not let executable text hide beside the redacted receipt command', () => {
    const needle = '--arg promotionCommand "npm dist-tag add @ashlr/hub@3.3.0 latest"';
    const mutated = promotionText.replace(
      needle,
      `${needle}; npm dist-tag add @ashlr/hub@3.3.0 latest`,
    );
