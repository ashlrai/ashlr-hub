...
const backend = vi.hoisted(() => ({ read: vi.fn(), validate: vi.fn(), hash: vi.fn(), setup: vi.fn(), run: vi.fn(), status: vi.fn() }));
...
vi.mock('../src/core/resources/engineering-mission-store.js', () => ({ validateResourceEngineeringMissionConfig: backend.validate, missionHash: backend.hash,
  readResourceEngineeringMissionStatus: backend.status }));
...
describe('engineering mission CLI', () => {
  it('reads recorded status without delivery verification, owner startup or signal handlers', async () => {
    const signals = [process.listeners('SIGINT'), process.listeners('SIGTERM')];
    backend.status.mockReturnValue({ scope: 'recorded-mission-evidence-only', recordedPhase: 'settled', ownerState: 'not-observed' });
    expect(await command(['status', '--config', '/private/mission.json', '--json'])).toBe(0);
    expect(backend.status).toHaveBeenCalledExactlyOnceWith(config); expect(backend.setup).not.toHaveBeenCalled();
    expect(backend.run).not.toHaveBeenCalled(); expect([process.listeners('SIGINT'), process.listeners('SIGTERM')]).toEqual(signals);
    expect(JSON.parse(String(output.mock.calls[0]![0]))).toMatchObject({ ownerState: 'not-observed' });
  });
  it('does not turn unavailable history into empty or completed status', async () => {
    backend.status.mockImplementation(() => { throw Error('PRIVATE_PATH'); });
    expect(await command(['status', '--config', '/private/mission.json', '--json'])).toBe(1);
    expect(String(output.mock.calls[0]![0])).not.toContain('PRIVATE_PATH'); expect(backend.setup).not.toHaveBeenCalled(); expect(backend.run).not.toHaveBeenCalled();
  });
  ...
  it.each([
    [], ['run'], ['check', '--execute'], ['status', '--config', '/private/mission.json', '--execute'], ['run', '--config', '/private/mission.json', '--execute'],
