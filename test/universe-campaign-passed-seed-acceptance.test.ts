...
import { verifiedInitialCampaignSeedImprovement } from '../src/core/universe/campaign-improvement.js';
...
describe.runIf(process.platform === 'darwin')('passed measured seed first-generation delivery acceptance', () => {
  it('delivers150 to149 with one real worker and replays without another generation', async () => {
    ...
    writeFileSync(join(repo, 'value.json'), '150\n');
    writeFileSync(join(repo, 'worker.mjs'), "import {readFileSync,writeFileSync} from 'node:fs';\n" +
      "if(process.env.ASHLR_UNIVERSE_GENERATION!=='1'||JSON.parse(readFileSync('value.json','utf8'))!==150)process.exit(3);\n" +
      "writeFileSync('value.json','149\\n');\n");
    writeFileSync(join(repo, 'evaluate.mjs'), "import {readFileSync} from 'node:fs';import {join} from 'node:path';\n" +
      ...
      "console.log(JSON.stringify({passed:value===150||value===149,score:value,metrics:{value,seed:process.env.ASHLR_UNIVERSE_EVALUATION_CONTEXT==='campaign-seed-v1'?1:0}}));\n");
    ...
    initUniverse({ schemaVersion: 1, id: 'fixture', name: 'Passed-seed delivery', objective: 'Improve a passing seed',
      ...
      variants: [{ id: 'candidate', niche: 'value', hypothesis: 'Reduce value once', command: [process.execPath, 'worker.mjs'] }] }, { root: store });
    initUniverseCampaign({ schemaVersion: 1, id: 'campaign', universeId: 'fixture', feedback: false, measureSeed: true,
      budget: { maxGenerations: 1, maxDurationMs: 40_000, maxModelRequests: 0, maxStagnantGenerations: 1, maxReportedTokens: null } }, { root: store });
    ...
    try {
      ...
      expect(result.campaign).toMatchObject({ state: 'completed', sourceState: 'healthy',
        seedEvaluation: { result: { status: 'measured', processGroupSettlement: 'group-exit-confirmed', reason: null,
          measurement: { passed: true, score: 150, metrics: { value: 150, seed: 1 } } } },
        progress: { attempts: 1, completedRuns: 1, reservedModelRequests: 0, admissions: 1, improvements: 0 } });
      expect(universe.runs).toHaveLength(1); expect(universe.runs[0]!.trials).toHaveLength(1);
      const trial = universe.runs[0]!.trials[0]!;
      expect(trial).toMatchObject({ status: 'passed', selected: true, score: 149, metrics: { value: 149, seed: 0 }, parentTrialId: null, delta: null });
      ...
      preserve = false;
      expect(result.delivery.status).toBe('delivered'); if (result.delivery.status !== 'delivered') throw new Error('Expected first candidate delivery');
      const receipt = result.delivery.receipt;
      expect(receipt.trialId).toBe(trial.id);
      expect(verifiedInitialCampaignSeedImprovement(universe, result.campaign, trial, manifestRecord(directory).seedArtifact.digest))
        .toMatchObject({ kind: 'passed-seed-evaluation', delta: 1 });
      expect(readCompletedCampaignDelivery(result.campaign, delivery, { root: store })).toEqual(receipt);
      expect(git('show', `${receipt.commit}:value.json`)).toBe('149');
      ...
      expect(readFileSync(join(repo, 'value.json'), 'utf8')).toBe('150\n');
      ...
      expect(readUniverseDeliveries('fixture', { root: store }).deliveries).toEqual([receipt]);
      expect(git('rev-parse', `refs/heads/${delivery.branch}`)).toBe(receipt.commit);
