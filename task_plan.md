...
- [x] Mark budget pauses with a typed non-ordinary gateway disposition and exclude them from ordinary-route eligibility.
...
- [x] Add a pending-only, replay-bound activity projection from the complete reader before connecting duplicate suppression callers.
- [x] Wire the coherent pending snapshot and activity map into the daemon's duplicate-suppression path with fail-closed fallback.
- [ ] Require the protected CI matrix on the immutable draft head before promotion.
...
- [x] Project exact V2 envelope/shared-binding joins as local-only Fleet Status evidence without learning or merge authority.
