/** Source-only fixed workload. Bundled into the installed entry; never candidate-selected.
 * Returns provisional diagnostic data. The outer owner alone completes activity
 * custody and publishes output after all owned groups are settled.
 */
...
// Fixture setup and the synchronous empty-home projection share process state.
// Refuse overlap rather than running two workloads against that singleton.
let active = false;
...
function fixture(base, count, gitPin) {
  const runGit = (repo, ...args) => git(gitPin, repo, ...args);
  ...
  for (const dir of [workspace, transport]) { fs.mkdirSync(dir, { mode: 0o700 }); runGit(dir, 'init', '-q', '--template=', '--initial-branch=main'); }
  ...
  runGit(workspace, 'add', '.'); runGit(workspace, '-c', 'user.name=Fixture', '-c', 'user.email=fixture@example.invalid', 'commit', '-qm', 'fixed seed');
  const revision = runGit(workspace, 'rev-parse', 'HEAD');
...
/** Trusted entry-only options; no caller-provided bridge, fixtures or callbacks. */
export async function runPreparationWorkload(context) {
  const installed = context?.mode === 'preparation-workflows-v2';
  const failureReport = () => ({ schemaVersion: 1, kind: 'preparation-verification-measurement',
    ...(installed ? { workload: 'preparation-workflows-v2', workflows: [], qualifications: [] } : {}),
    checksPassed: false, metrics: { correctness_checks: 0 }, diagnostics: [{ code: 'HARNESS_INITIALIZATION_FAILED',
      message: installed ? 'Pinned verification workload did not satisfy its fixed checks at initialize.'
        : 'Pinned verification prototype did not satisfy its fixed checks.' }] });
  if (active) return failureReport();
  active = true;
  try { return await evaluate(context); }
  finally { active = false; }
}

async function evaluate(context) {
  let failure = 'HARNESS_INITIALIZATION_FAILED'; let checkpoint = 'initialize';
  const metrics = {}; const workflows = []; const qualifications = []; const qualificationFixtures = []; let checks = 0; let session;
  const workload = context?.mode;
  let activity;
  try {
    assert.ok(context && typeof context === 'object');
    assert.ok(workload === 'preparation-leaf-v1' || workload === 'preparation-workflows-v2');
    const { candidateRoot, scratchRoot, signal, deadlineAt, deadlineMonotonicMs: monotonicDeadline } = context;
    activity = context.activity;
    assert.ok(signal instanceof globalThis.AbortSignal);
    if (workload === 'preparation-workflows-v2') {
      assert.ok(activity && typeof activity.lifecycle === 'function');
      assert.ok(Number.isFinite(deadlineAt) && Number.isFinite(monotonicDeadline));
      assert.equal(Date.parse(activity.owner.deadlineAt), deadlineAt);
    } else {
      assert.equal(activity, undefined);
      assert.equal(deadlineAt, Infinity); assert.equal(monotonicDeadline, Infinity);
    }
    const remaining = () => Math.floor(Math.min(deadlineAt - Date.now(), monotonicDeadline - performance.now()));
    function guard() { if (signal.aborted || remaining() <= 0) throw new Error('MEASUREMENT_CANCELLED_OR_EXPIRED'); }
    guard();
    assertPreparationGit(context.gitPin);
    const gitPin = Object.freeze({ path: context.gitPin.path, digest: context.gitPin.digest });
    guard();
    ...
    assert.ok(typeof scratchRoot === 'string' && path.isAbsolute(scratchRoot) && fs.realpathSync(scratchRoot) === scratchRoot);
    failure = 'FIXTURE_SETUP_FAILED';
    const base = fs.mkdtempSync(path.join(scratchRoot, 'verification-'));
    for (const count of [1, 4]) {
      guard();
      failure = 'FIXTURE_SETUP_FAILED';
      const f = fixture(base, count, gitPin);
      ...
      session = await createPreparationCandidateSession({ bridge, bridgePath, candidateRoot, fixtureRoot: f.root, workRoot: base,
        timeoutMs, signal, activity, gitPin });
    ...
    if (activity) {
      ...
      for (const kind of ['manager', 'successor']) {
        ...
        const setup = await createPreparationWorkflowFixture(kind, fixtureRoot, { activity, signal,
        ...
        session = await createPreparationCandidateSession({ bridge, bridgePath, candidateRoot, fixtureRoot, workRoot: base,
          timeoutMs: Math.min(MAX_SESSION_DURATION_MS, remaining()), signal, activity, gitPin });
      ...
      for (const item of qualificationFixtures) {
        ...
        const row = await qualifyPreparationCandidate({ ...item, bridge, bridgePath, candidateRoot, workRoot: base,
          activity, signal, gitPin, deadlineAt, deadlineMonotonicMs: monotonicDeadline });
