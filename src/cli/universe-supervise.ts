...
function parse(args: string[]): { ids: string[]; options: UniverseCampaignSupervisorOptions; json: boolean } {
  const ids: string[] = []; const seen = new Set<string>();
  const values: Record<string, string> = {}; let json = false;
  const names = ['--root', '--resource-runtime', '--max-duration-ms', '--max-concurrent', '--poll-interval-ms'];
  ...
  return { ids, json, options: { root, maxDurationMs: integer('--max-duration-ms', 1, 86_400_000),
...
export async function cmdUniverseSupervise(args: string[]): Promise<number> {
  ...
  try {
    const { ids, options, json } = parse(args);
