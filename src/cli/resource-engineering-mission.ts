...
const USAGE = `usage: ashlr resources pool engineering mission check --config ABS [--json]
       ashlr resources pool engineering mission status --config ABS [--json]
...
Check and status are read-only. Status projects recorded progress and remaining
time; it does not revalidate deliveries or prove an owner is running.
Run is effectful: it can start configured collectors and
...
Exit codes: 0 observed/checked/completed/help, 1 held/stopped/unavailable, 2 invalid arguments.
`;
export async function cmdResourceEngineeringMission(args: string[]): Promise<number> {
  try {
    ...
    if (!['check', 'status', 'run'].includes(mode ?? '') || executes !== (mode === 'run' ? 1 : 0)) throw new ResourceCheckUsageError('Choose check/status, or run with --execute');
    ...
    const [{ readResourceJson }, { validateResourceEngineeringMissionConfig, missionHash, readResourceEngineeringMissionStatus }] = await Promise.all([
      import('../core/resources/pool-runtime.js'), import('../core/resources/engineering-mission-store.js'),
    ]);
    ...
    if (mode === 'status') { console.log(JSON.stringify(readResourceEngineeringMissionStatus(config))); return 0; }
    const { checkResourceEngineeringAutonomousSetup } = await import('../core/resources/engineering-autonomous-setup.js');
