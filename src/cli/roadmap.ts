 ...
 *   ashlr roadmap resume [--dir <d>] [--roadmap <file>] [--json]   # alias for run
...
function parseFlags(args: string[]): ParsedFlags {
  ...
  return { dir, roadmapFile, dryRun, json };
}
...
export const cmdRoadmap = async (args: string[]): Promise<number> => {
  ...
  if (!sub || sub === 'help' || sub === '--help' || sub === '-h') {
    console.log('Usage: ashlr roadmap <run|resume|status> [--dir <d>] [--roadmap <f>] [--dry-run] [--json]');
    return sub ? 0 : 2;
  }
