...
const HELP = `Usage: ashlr universe firm <demo|status|query> --root <existing-private-dir> [--json]
       ashlr universe firm query --root <existing-private-dir> [--entity <id>] [--limit <1..256>] [--json]

Runs only a fixed, inert plan -> artifact -> cold-check -> signed-trace fixture.
...
export async function cmdUniverseFirm(args: string[]): Promise<number> {
  if (!args.length || args.length === 1 && ['help', '--help', '-h'].includes(args[0]!) ||
      args.length === 2 && ['demo', 'status', 'query'].includes(args[0]!) && ['--help', '-h'].includes(args[1]!)) {
    console.log(HELP); return 0;
  }
  const command = args[0];
  if (!command || !['demo', 'status', 'query'].includes(command)) { console.error(HELP); return 2; }
  let root: string | undefined; let json = false;
  const query: { entity?: string; limit?: number } = {};
  ...
  for (let index = 1; index < args.length; index++) {
    ...
    if (!['--root', ...(command === 'query' ? ['--entity', '--limit'] : [])].includes(flag)) {
  ...
  try {
    ...
    const error = { schemaVersion: 1, status: 'unavailable', reason: 'firm-demo-input-or-evidence-unavailable' };
    if (json) console.log(JSON.stringify(error)); else console.error('Firm demo unavailable: check the explicit private root and existing evidence.');
    return 1;
  } finally {
