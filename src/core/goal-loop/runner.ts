 ...
 *
 * M86: ported from the 0.1.0 line (backup/local-0.1.0-work) and adapted to the
 * v3 fleet/engine architecture. The port is faithful; the v3 adaptations are:
 *  - spawnEngine is async (M236 streaming) — awaited, with the foundry
 *    timeout backstop;
 *  - engineInstalled takes cfg (config-declared M50 engines resolve);
 *  - local-first hard rule: api-model (cloud API) engines are refused unless
 *    the operator passes --allow-cloud. The cli-agent engines (claude/codex)
 *    run on the operator's own installed subscription auth.
 *
...
/**
 * Per-milestone spawn backstop. Milestones are substantial work — mirror the
 * sandboxed-engine 2h runaway-cost safety net; cfg.foundry.timeoutMs wins.
 */
const DEFAULT_MILESTONE_TIMEOUT_MS = 2 * 60 * 60_000;
 ...
 *
 * Engine selection: ctx.engine (--engine flag) → `ASHLR_ENGINE` → `claude`;
 * the model honors `ASHLR_MODEL` / `AC_MODEL`, matching the orchestrator's
 * resolution. `builtin` is rejected: it runs in-process and so cannot provide
 * the per-milestone context reset this loop depends on. M86 local-first rule:
 * api-model (cloud API) engines require ctx.allowCloud (--allow-cloud).
 */
export const defaultRunMilestone: RunMilestoneFn = async (doc, ctx) => {
  const engine = ctx.engine ?? ((process.env['ASHLR_ENGINE'] as EngineId | undefined) ?? 'claude');
  ...
  // M86 hard rule: local-first. cli-agent engines (claude/codex) run on the
  // operator's own installed subscription auth; api-model engines POST to a
  // remote endpoint and need the explicit opt-in.
  if (resolveEngineSpec(engine, ctx.cfg)?.kind === 'api-model' && !ctx.allowCloud) {
    return blocked(
      doc.id,
      `engine "${engine}" is a cloud API backend — pass --allow-cloud to permit it (local-first rule)`,
    );
  }
  if (!engineInstalled(engine, ctx.cfg)) {
  ...
  const res = await spawnEngine(cmd, ctx.cfg, {
    timeoutMs: ctx.cfg.foundry?.timeoutMs ?? DEFAULT_MILESTONE_TIMEOUT_MS,
  });
  if (!res.ok) {
