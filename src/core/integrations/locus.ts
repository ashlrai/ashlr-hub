/**
 * src/core/integrations/locus.ts — Locus identity-plane probe for ashlr-hub.
 *
 * Landed from Locus `integrations/ashlr-hub/locus.ts`. Shells out to the
 * `locus` CLI; never reimplements seal/pin.
 *
 * SECURITY:
 ...
 *   locusWatchOnce, ensureLocusReady, locusFleetGate,
 ...
 * @see locus docs/verification-plane.md
 */
 ...
 * Accepts either `cfg.locus`, a bare `{ enforce?, firm? }`, or a full
 * {@link AshlrConfig}.
 */
export type LocusEnforceConfigInput =
  | Pick<AshlrConfig, "locus">
  | NonNullable<AshlrConfig["locus"]>
  | AshlrConfig
  | null
  | undefined;
...
export function parseWatchHeartbeat(
  ...
  if (
    ...
    const safeNext =
      ...
      "unknown";
    const pinned =
      coerceBool(obj.pinned) ||
      (whoami != null && whoami.length > 0);
 ...
 * accepting AshlrConfig / `{ locus }` / bare locus objects.
 */
function locusSliceFromConfig(
  config?: LocusEnforceConfigInput,
): NonNullable<AshlrConfig["locus"]> | undefined {
  if (config == null || typeof config !== "object") return undefined;
  // AshlrConfig or { locus?: { ... } }
  if ("locus" in config) {
    const locus = (config as { locus?: unknown }).locus;
    if (locus != null && typeof locus === "object") {
      return locus as NonNullable<AshlrConfig["locus"]>;
    }
    return undefined;
  }
  // Bare { enforce?, firm? } (or any object without a locus key)
  return config as NonNullable<AshlrConfig["locus"]>;
}
 ...
 * Absent / false / non-boolean → false (monorepo-safe).
 */
 ...
 */
export function readLocusConfigFromAshlr():
  | NonNullable<AshlrConfig["locus"]>
  | undefined {
  try {
    const cfg = loadConfigReadOnly();
    const locus = cfg?.locus;
    if (locus == null || typeof locus !== "object") return undefined;
    return locus;
  } catch {
    return undefined;
  }
...
export function assertLocusPreMutate(
  ...
  const cfg =
    arguments.length >= 2 ? config : readLocusConfigFromAshlr();
