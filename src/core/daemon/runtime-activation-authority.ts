...
export const RUNTIME_ACTIVATION_MANIFEST_DOMAIN_V1 =
  'ashlr:runtime-activation-manifest:v1' as const;
export const RUNTIME_ACTIVATION_REQUEST_DOMAIN_V1 =
  'ashlr:runtime-activation-preflight-request:v1' as const;
export const RUNTIME_ACTIVATION_PLAN_EVIDENCE_DOMAIN_V1 =
  'ashlr:runtime-activation-plan-evidence:v1' as const;
...
export interface RuntimeActivationAuthorityPreflightResult {
  ...
  authorityBlockers: readonly [
    'transactional-install-consumer-absent',
    'descriptor-bound-launch-consumer-absent',
    'durable-replay-consumption-absent',
    'signed-revision-provenance-consumer-absent',
    'rollback-execution-consumer-absent',
    'protected-postmerge-evidence-unbound',
    'external-monotonic-policy-anchor-absent',
  ];
}
...
function validTimestamp(value: unknown): value is string {
  return typeof value === 'string' && value.length >= 20 && value.length <= 32 &&
...
function validateArtifactBinding(value: unknown): RuntimeActivationArtifactBindingV1 {
  if (!isRecord(value) || !exactKeys(value, [
    'dependencyInventoryDigest', 'envelopeCanonicalSha256', 'envelopeSha256',
    'evidenceKeyId', 'evidenceTrustRootCanonicalSha256', 'evidenceTrustRootSha256',
    'expectedRevision', 'expectedTree', 'independentlyPackaged', 'interpreterSha256',
    'manifestDigest', 'packageTarballSha256', 'policyId', 'runtimeTreeSha256',
    'serviceDescriptorSha256', 'serviceInvocationDigest',
  ])) throw new Error('activation artifact binding has invalid keys');
  for (const key of [
    'dependencyInventoryDigest', 'envelopeCanonicalSha256', 'envelopeSha256',
    'evidenceTrustRootCanonicalSha256', 'evidenceTrustRootSha256', 'interpreterSha256',
    'manifestDigest', 'packageTarballSha256', 'runtimeTreeSha256',
    'serviceDescriptorSha256', 'serviceInvocationDigest',
  ] as const) {
  ...
  if (typeof value['expectedRevision'] !== 'string' || !SHA1_RE.test(value['expectedRevision']) ||
    typeof value['expectedTree'] !== 'string' || !SHA1_RE.test(value['expectedTree']) ||
    typeof value['evidenceKeyId'] !== 'string' || !KEY_ID_RE.test(value['evidenceKeyId']) ||
    typeof value['policyId'] !== 'string' || !/^sha256:[a-f0-9]{64}$/.test(value['policyId']) ||
    value['independentlyPackaged'] !== true) {
    throw new Error('activation artifact identity is invalid');
  }
  return value as unknown as RuntimeActivationArtifactBindingV1;
}
...
export function parseSignedRuntimeActivationManifest(
  value: string,
): ParsedActivationManifest {
  ...
  if (!isRecord(parsed) || !exactKeys(parsed, [
    ...
    typeof parsed['keyId'] !== 'string' || !KEY_ID_RE.test(parsed['keyId']) ||
    decodeBase64url(parsed['signature'], 64) === null || !isRecord(parsed['payload'])) {
    throw new Error('signed activation manifest is invalid');
  }
  const payload = parsed['payload'];
  if (!exactKeys(payload, [
    'activationMode', 'candidate', 'expiresAt', 'issuedAt', 'planId', 'policyEpoch', 'rollback',
  ]) || payload['activationMode'] !== 'resident-canary' ||
    !validTimestamp(payload['issuedAt']) || !validTimestamp(payload['expiresAt']) ||
    typeof payload['planId'] !== 'string' || !UUID_RE.test(payload['planId']) ||
...
function contained(anchor: string, candidate: string, allowEqual = false): boolean {
  ...
  return (allowEqual && nested === '') || (nested !== '' && nested !== '..' &&
    !nested.startsWith(`..${sep}`) && !isAbsolute(nested));
}

function verifyImmutableBundlePath(
  ...
  if (kind === 'file') {
    ...
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1n ||
      !ownedByCurrentUser(stat) || (process.platform !== 'win32' && (stat.mode & 0o222n) !== 0n) ||
...
function verifyPrivateAuthorityPath(path: string, homePath: string, kind: 'directory' | 'file'): void {
  ...
  const components = relative(home, kind === 'file' ? dirname(target) : target).split(sep).filter(Boolean);
  let current = home;
  const homeStat = lstatSync(home, { bigint: true });
  if (!homeStat.isDirectory() || homeStat.isSymbolicLink() || !ownedByCurrentUser(homeStat) ||
...
function loadOperatorTrustRoot(homePath: string): LoadedTrustRoot {
  ...
  if (!isRecord(parsed) || !exactKeys(parsed, [
    'assurance', 'domain', 'evidenceTrustRoot', 'minimumPolicyEpoch',
    'permittedActivationModes', 'schemaVersion',
  ]) || parsed['assurance'] !== 'operator-custodied-public-trust-root' ||
    parsed['domain'] !== RUNTIME_ACTIVATION_AUTHORITY_TRUST_ROOT_DOMAIN_V1 ||
    parsed['schemaVersion'] !== 1 || !isPositiveSafeInteger(parsed['minimumPolicyEpoch']) ||
  ...
  return {
    ...
    evidenceTrustRootCanonicalSha256:
      runtimeReleaseTrustRootCanonicalSha256(evidence.canonicalJson)!,
...
function validateBundleRequest(value: unknown): RuntimeActivationBundleRequestV1 {
  if (!isRecord(value) || !exactKeys(value, [
    'argv', 'bundleRoot', 'declaredInterpreterPath', 'declaredInterpreterVersion',
    'dependencyRoot', 'envelopePath', 'executablePath', 'manifestPath', 'packageRoot',
    'packageTarballPath', 'policyPath', 'serviceDescriptorPath',
  ])) throw new Error('activation bundle request has invalid keys');
  for (const key of [
    'bundleRoot', 'declaredInterpreterPath', 'declaredInterpreterVersion', 'dependencyRoot',
    'envelopePath', 'executablePath', 'manifestPath', 'packageRoot', 'packageTarballPath',
    'policyPath', 'serviceDescriptorPath',
  ] as const) {
  ...
  for (const key of [
    'bundleRoot', 'declaredInterpreterPath', 'dependencyRoot', 'envelopePath',
    'executablePath', 'manifestPath', 'packageRoot', 'packageTarballPath',
    'policyPath', 'serviceDescriptorPath',
  ] as const) {
...
function readRequest(requestPath: string, homePath: string): ReadRequestResult {
  ...
  if (dirname(canonicalRequestPath) !== paths.plansPath || realpathSync(canonicalRequestPath) !== canonicalRequestPath) {
  ...
  if (!isRecord(parsed) || !exactKeys(parsed, [
    'candidate', 'domain', 'rollback', 'schemaVersion', 'signedManifest',
  ]) || parsed['domain'] !== RUNTIME_ACTIVATION_REQUEST_DOMAIN_V1 ||
    parsed['schemaVersion'] !== 1 || !isRecord(parsed['signedManifest'])) {
...
function verifyActivationManifest(
  parsed: ParsedActivationManifest,
  ...
  if (publicKey.asymmetricKeyType !== 'ed25519' || runtimeReleaseEvidenceKeyId(publicKey) !== manifest.keyId ||
...
function hashStableArtifact(path: string, rootPath: string): string {
  ...
  try {
    ...
    if (!sameSnapshot(openedBefore, openedAfter) || !sameSnapshot(openedAfter, fileAfter) ||
      !sameSnapshot(rootBefore, rootAfter) || realpathSync(root) !== root || realpathSync(target) !== target) {
...
function observeBundle(
  ...
  if (!equalDigest(hashStableArtifact(request.packageTarballPath, bundleRoot), binding.packageTarballSha256) ||
  ...
  if (runtimeReleaseEnvelopeCanonicalSha256(envelope) !== binding.envelopeCanonicalSha256 ||
  ...
  if (!parsedEnvelope.ok || !parsedManifest.ok || parsedEnvelope.envelope.keyId !== binding.evidenceKeyId ||
  ...
  return { envelope, manifest, launchReceiptSha256: sha256(observed.canonicalJson) };
}
...
function emptyResult(blockers: RuntimeActivationPreflightBlocker[]): RuntimeActivationAuthorityPreflightResult {
  return {
    ...
    startPermitted: false,
    executionPerformed: false,
    plan: { planId: null, planDigest: null, replayKey: null, policyEpoch: null, activationMode: null, expiresAt: null },
    trust: { operatorCustodyVerified: false, signatureVerified: false, signingKeyId: null, trustRootCanonicalSha256: null },
    releases: { candidateRevision: null, candidateTree: null, rollbackRevision: null, rollbackTree: null, pairVerified: false },
    blockers,
    authorityBlockers: [
      'transactional-install-consumer-absent',
      'descriptor-bound-launch-consumer-absent',
...
export function preflightRuntimeActivationAuthority(input: {
  ...
  if (blockers.length === 0) {
    try { candidate = observeBundle(read.request.candidate, payload.candidate, trust); } catch (error) {
      blockers.push({ code: 'candidate-artifact-invalid', detail: error instanceof Error ? error.message : 'candidate artifact is invalid' });
    }
    try { rollback = observeBundle(read.request.rollback, payload.rollback, trust); } catch (error) {
      blockers.push({ code: 'rollback-artifact-invalid', detail: error instanceof Error ? error.message : 'rollback artifact is invalid' });
    }
  }
  const distinct = payload.candidate.expectedRevision !== payload.rollback.expectedRevision &&
  ...
  if (!distinct) blockers.push({
    code: 'rollback-not-independent',
    detail: 'candidate and rollback must be distinct revisions, trees, manifests, tarballs, runtime trees, and bundle paths',
  });
  ...
  return {
    ...
    trust: {
      ...
      signatureVerified: blockers.every((entry) => ![
        'signed-manifest-invalid', 'signed-manifest-expired', 'policy-epoch-stale', 'activation-mode-denied',
      ].includes(entry.code)),
...
export function runtimeActivationPlanEvidenceCanonicalJson(
  input: RuntimeActivationPlanEvidenceV1,
): string {
  if (!UUID_RE.test(input.planId) || !SHA256_RE.test(input.planDigest) ||
    !SHA256_RE.test(input.replayKey) || !SHA256_RE.test(input.trustRootCanonicalSha256) ||
    !isPositiveSafeInteger(input.policyEpoch) || input.activationMode !== 'resident-canary' ||
    input.authority !== 'observation-only' || input.executionPermitted !== false ||
    input.domain !== RUNTIME_ACTIVATION_PLAN_EVIDENCE_DOMAIN_V1 || input.schemaVersion !== 1) {
...
function parseRuntimeActivationPlanEvidence(value: unknown): RuntimeActivationPlanEvidenceV1 | null {
  if (!isRecord(value) || !exactKeys(value, [
    'activationMode', 'authority', 'domain', 'executionPermitted', 'planDigest',
    'planId', 'policyEpoch', 'replayKey', 'schemaVersion', 'trustRootCanonicalSha256',
  ])) return null;
  try {
...
function runtimeActivationPlanEvidenceCodec(): ImmutablePrivateRecordCodec<RuntimeActivationPlanEvidenceV1> {
  return {
    ...
    isRecordFileName: (fileName) => fileName.endsWith('.json') &&
      UUID_RE.test(fileName.slice(0, -'.json'.length)),
    stageToken: (record) => record.replayKey,
    equivalent: (left, right) =>
      runtimeActivationPlanEvidenceCanonicalJson(left) ===
      runtimeActivationPlanEvidenceCanonicalJson(right),
  };
}

function runtimeActivationPlanEvidenceStore(input: {
  anchorPath: string;
  storeRoot: string;
}) {
  return {
...
export function readRuntimeActivationPlanEvidence(
  ...
  return readImmutablePrivateRecordPoint(
    runtimeActivationPlanEvidenceStore(storage),
    planId,
    `${planId}.json`,
  );
}
