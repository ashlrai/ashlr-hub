 ...
 *                             (DEFAULT_LOCAL_FLEET_MAX_DISPATCHES_PER_DAY, and
 *                             see the settings reader below) — a fleet with no
 *                             ceiling at all is exactly the unbounded loop this
 *                             section exists to refuse. `null` means unbounded
 *                             and is honoured ONLY when the operator wrote null
 *                             explicitly; an unbounded ceiling is then reported
 *                             as unbounded rather than quietly implied to be
 *                             safe.
 *
