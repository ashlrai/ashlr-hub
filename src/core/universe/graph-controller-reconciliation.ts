...
interface GraphControllerReconciliationOptions {
  root: string;
...
export function reconcileGraphControllerDispatch(options: GraphControllerReconciliationOptions): UniversePortfolioControllerReport | null {
  ...
  try {
    ...
    const guard = (events: readonly PortfolioControllerEvent[]): void => {
    ...
    let events = readPortfolioControllerEvents(directory); guard(events);
    ...
    events = readPortfolioControllerEvents(directory); guard(events); owned();
    const initial = foldPortfolioController(events);
    if ([...initial.states.values()].some(row => row.state !== 'completed' && row.state !== 'in-flight')) return null;
