...
function campaignAdmission(id: string, options: CampaignOptions): { initial: UniverseCampaignSummary; dispatchId: string | undefined } {
...
export async function runUniverseCampaign(id: string, options: CampaignOptions = {}): Promise<UniverseCampaignSummary> {
  const { initial, dispatchId } = campaignAdmission(id, options);
  if (terminalCampaign(initial.state)) return initial;
  return await withUniverseExecution(initial.definition.universeId, options,
    (lock) => runCampaignWithLease(id, options, lock, dispatchId, initial.definition.universeId));
}

/** Internal admission seam: the caller already owns and remains responsible for
 * releasing this exact experiment lease. It never reacquires or releases ownership.
 */
export async function runUniverseCampaignOwned(id: string, options: CampaignOptions,
  lock: LocalStoreLock): Promise<UniverseCampaignSummary> {
  const { initial, dispatchId } = campaignAdmission(id, options);
  assertUniverseExecution(universePath(resolve(options.root ?? defaultUniverseRoot()), initial.definition.universeId), lock);
  if (terminalCampaign(initial.state)) return initial;
  return await runCampaignWithLease(id, options, lock, dispatchId, initial.definition.universeId);
}

async function runCampaignWithLease(id: string, options: CampaignOptions, lock: LocalStoreLock,
  dispatchId: string | undefined, universeId: string): Promise<UniverseCampaignSummary> {
  const executionDirectory = universePath(resolve(options.root ?? defaultUniverseRoot()), universeId);
  const owned = (): void => assertUniverseExecution(executionDirectory, lock);
  owned();
  const directory = campaignDirectory(id, options);
  ...
  const finish = (state: Settlement, reason: string, checkpoint?: string): UniverseCampaignSummary => {
    owned();
    return settle(id, state, reason, options, checkpoint, ownedDispatchId);
  };
