...
export interface AshlrConfig {
  ...
  foundry?: {
     ...
     *
     * When explicitly true, routeTask computes a per-(engine, model, taskClass)
     ...
     *
     * False or absent preserves exact pre-M240 behavior (byte-identical static
     * routing). Operational steering additionally requires causally
     * identifiable, authenticated assignment evidence.
     */
 ...
 * A dollar cap cannot bound local work. A local dispatch costs $0, so a USD
 * guard never fires and "bounded by budget" becomes a comfortable fiction
 * wrapped around an unbounded loop — `src/core/daemon/local-fleet.ts` states
 * this outright and names the honest limiters. Every ceiling here is therefore
 * NON-MONETARY and bounds a run whether it ran on a frontier API or entirely
 * on this machine.
 *
 * The dimensions are INDEPENDENT and each is sufficient on its own: a run may
 * be bounded by tokens, by iterations, by a deadline, or by any combination.
 * An ABSENT optional ceiling means "this dimension does not bound the run" —
 * it never means zero. `budgetVerdict()` in `src/core/run/budget.ts` is the
 * single pure predicate that reads them.
 */
export interface RunBudget {
  ...
  allowCloud: boolean;
  /**
   * Maximum COMPLETED iterations (daemon ticks / dispatch rounds) before the
   * run stops. Distinct from `maxSteps`, which counts model invocations inside
   * one run; this counts how many times an outer loop came back for more.
   * Absent ⇒ iterations do not bound this run.
   */
  maxIterations?: number;
  /**
   * Absolute wall-clock stop time as epoch milliseconds. The run is exhausted
   * at or after this instant. Absent ⇒ no absolute deadline.
   */
  deadlineEpochMs?: number;
  /**
   * Maximum wall-clock duration in milliseconds measured from the run's start.
   * Requires the caller to supply `startedAtMs`; see `RunProgress`.
   * Absent ⇒ no duration ceiling.
   */
