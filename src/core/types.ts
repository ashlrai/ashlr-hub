...
export interface WebServerOptions {
  ...
  /** Server-owned capability scope for the generated local session token. */
  mutationRole?: 'observer' | 'operator' | 'approver' | 'owner';
  /**
   * Require a persisted metadata-only authority receipt before web mutations.
   * Deprecated compatibility option. Dispatch-enabled servers always require
   * receipts regardless of this value.
   */
  requireMutationAuditReceipts?: boolean;
}
...
export interface AuditEntry {
  /** Additive writer schema. Absent on legacy records. */
  schemaVersion?: 2;
  /** Opaque correlation id generated by `audit()` after a successful append. */
  eventId?: string;
  ...
  /** Authenticated actor metadata when the caller has established identity. */
  actor?: {
    id: string;
    type: 'human' | 'agent' | 'system' | 'unknown';
    role: string;
  };
  /** Closed authority metadata. Never request bodies, prompts, diffs, or tokens. */
  authority?: {
    method: string;
    capability: string;
  ...
  /** Privacy-safe binding for a reserved or completed web mutation. */
  mutation?: {
