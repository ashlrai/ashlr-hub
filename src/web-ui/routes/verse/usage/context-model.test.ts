...
describe('picker and dialog wording', () => {
  ...
  it('explains what each mode costs with arithmetic, not adjectives', () => {
    ...
    expect(expansive).toContain('re-sends 2.6× what one at 367k does');
    expect(expansive).not.toContain('OpenAI');
    expect(contextModeDescription('codex', GPT6, 'expansive')).toContain('OpenAI reportedly');
...
describe('context fit', () => {
  it('a 300k working set needs expansive on GPT-6 and must be split on a 64k local model', () => {
    expect(modelFit(300_000, GPT6)).toBe('expansive');
    expect(modelFit(300_000, LOCAL)).toBe('split');
    expect(modelFit(300_000, FABLE)).toBe('tight');
    expect(modelFit(100_000, FABLE)).toBe('fits');
    expect(modelFit(300_000, GPT55)).toBe('split');
    // Unknown budget, unknown working set: no verdict, never a guess.
    expect(modelFit(300_000, MYSTERY)).toBeNull();
    expect(modelFit(null, FABLE)).toBeNull();
    expect(modelFit(Number.NaN, FABLE)).toBeNull();
  });
  ...
  it('judges a chat set to Expansive against the budget it will actually run with', () => {
    expect(modelFit(300_000, FABLE, 'expansive')).toBe('fits');
    expect(modelFit(700_000, FABLE, 'expansive')).toBe('tight');
    expect(modelFit(2_000_000, FABLE, 'expansive')).toBe('split');
    expect(modelFit(300_000, GPT6, 'expansive')).toBe('fits');
    // No expansive budget: the mode resolves to Standard, and so does the verdict.
    expect(modelFit(300_000, HAIKU, 'expansive')).toBe('split');
...
describe('idleCacheWarning', () => {
  ...
  it('warns once the chat has been idle past the cache lifetime (61 minutes)', () => {
    expect(idleCacheWarning(session(), 142_000, at + 61 * 60_000)).toEqual({ idleMs: 61 * 60_000, tokens: 142_000, local: false });
    expect(idleCacheWarning(session(), 142_000, at + CACHE_IDLE_TTL_MS - 60_000)).toBeNull();
  });

  it('stays quiet while running, for a context no bigger than a fresh chat’s, and on a bad timestamp', () => {
    expect(idleCacheWarning(session({ status: 'running' }), 142_000, at + 2 * CACHE_IDLE_TTL_MS)).toBeNull();
    expect(idleCacheWarning(session(), 20_000, at + 2 * CACHE_IDLE_TTL_MS)).toBeNull();
    expect(idleCacheWarning(session({ updatedAt: 'not a date' }), 142_000, at + 2 * CACHE_IDLE_TTL_MS)).toBeNull();
  });

  it('marks a local chat, whose cost is time rather than spend', () => {
    expect(idleCacheWarning(session({ engine: 'local' }), 40_000, at + 2 * CACHE_IDLE_TTL_MS)?.local).toBe(true);
  });
});
