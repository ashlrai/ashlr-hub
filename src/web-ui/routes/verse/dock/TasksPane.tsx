...
export function TasksPane({ turnTasks, otherChats, hasSession, onOpenSession }: TasksPaneProps) {
  const newChatKey = findCommand('chat.new')?.keys[0];
  const newChat = newChatKey ? formatChord(newChatKey) : null;
      ...
      <section className={styles.section} aria-labelledby="dock-tasks-turn">
        ...
        {!hasSession ? (
          ...
          <>
            ...
            <p className={styles.hint}>Send a message: each tool the agent runs shows up here, and a click jumps to it in the chat.</p>
          </>
        ) : (
            ...
            {[...running, ...finished].map((task) => (
              <li key={task.toolUseId}>
                <button type="button" className={styles.task} data-status={task.status}
                  ...
                  <span className={styles.taskName} title={task.kind === 'subagent' ? 'Subagent' : task.name}>{task.kind === 'subagent' ? 'Subagent' : task.name}</span>
        ...
        {otherChats.length === 0 ? (
          <>
            ...
            <p className={styles.hint}>
              {newChat ? `Start another with ${newChat}` : 'Start another chat'} and it shows here while it works, one click away.
            </p>
          </>
        ) : (
          <ul className={styles.taskList}>
            {otherChats.map((chat) => (
              <li key={chat.sessionId}>
                <button type="button" className={styles.task} data-status="running" onClick={() => onOpenSession(chat.sessionId)}
                  ...
                  <span className={styles.taskName} title={chat.title || 'Untitled chat'}>{chat.title || 'Untitled chat'}</span>
