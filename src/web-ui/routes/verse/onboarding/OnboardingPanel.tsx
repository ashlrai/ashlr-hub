...
function describe(completedAt: string | null, dismissedAt: string | null): string {
  if (completedAt) return `Last completed ${new Date(completedAt).toLocaleDateString()}.`;
  ...
  return (
    <Panel title="Onboarding">
      <SettingRow
        ...
        description={`Your connected seats, whether a local runtime is available, and what each of the three stop controls really halts. ${describe(completedAt, dismissedAt)}`}
      >
