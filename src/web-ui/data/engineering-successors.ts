...
export function decodeEngineeringSuccessors(value: unknown): Snapshot {
  if (!object(value) || !exact(value, ['schemaVersion', 'supervisionId', 'profileId', 'configDigest', 'deadlineAt', 'state', 'maxSuccessors', 'entries']) ||
    ...
    typeof value.state !== 'string' || !states.includes(value.state) || !Number.isSafeInteger(value.maxSuccessors) ||
  ...
  for (const row of value.entries) {
    if (!object(row) || !exact(row, ['sourceEnrollmentId', 'proposalTaskId', 'successorId', 'state', 'reason']) || !id(row.sourceEnrollmentId) ||
      ...
      typeof row.state !== 'string' || !entryStates.includes(row.state) ||
      !(row.reason === null || typeof row.reason === 'string' && Object.hasOwn(engineeringSuccessorReasons, row.reason)) ||
